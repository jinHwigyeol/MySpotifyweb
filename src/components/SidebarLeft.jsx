        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {playlists.map(pl => {
            ...
            return (
              ...
              >
                <img src={pl.images?.[0]?.url} className="w-12 h-12 rounded shadow-lg object-cover" alt="p" />
                <div className="flex flex-col min-w-0">
                  <span className={`text-[16px] font-bold truncate ${isPlaying ? 'text-spotify' : 'text-gray-200'}`}>{pl.name}</span>
                  <span className="text-[14px] text-gray-500 truncate">플레이리스트 • {pl.owner.display_name}</span>
                </div>
              </div>
            );
          })}
